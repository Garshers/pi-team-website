...
// Use unique keys for localStorage based on formId
...
const handleSubmit = (event) => {
    ...
    console.warn('Zbyt szybka próba wysłania formularza lub osiągnięto limit. Spróbuj ponownie.');
    return;
    }
    ...
    .then((result) => {
        ...
        setTimeout(() => setMessageSent(false), 20000); // Hide success message after 20 seconds
...
return (
    ...
    <form id={formId} className="Form" onSubmit={handleSubmit}>
        ...
        <input
            type="checkbox"
            id={`privacyPolicy_${formId}`} // Unique ID for each checkbox
            name="privacyPolicy"
